#!/usr/bin/env python3
"""
OneSearch - Data Extraction Tool

Usage:
    python extract.py '{"urls": ["https://example.com"], "prompt": "Extract main topics"}'
...
def main():
    ...
    if not server_url:
        ...
        print('\n使用例:', file=sys.stderr)
        print('  python extract.py \'{"urls": ["https://example.com"], "prompt": "Extract"}\'', file=sys.stderr)
        sys.exit(1)

    # Parse arguments
    if len(sys.argv) < 2:
        print('❌ エラー: 抽出パラメータが必要です', file=sys.stderr)
        print('\n使用例:', file=sys.stderr)
        print('  python extract.py \'{"urls": ["https://example.com"], "prompt": "Extract main topics"}\'', file=sys.stderr)
        sys.exit(1)
    ...
    # Make HTTP request
    url = f"{server_url.rstrip('/')}/api/tools/extract"
